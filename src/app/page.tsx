...
const questions: QuizQuestion[] = [
  ...
  },
  {
    ...
    options: [
      { text: 'Залипаю в идеи, как упростить что-то с помощью технологий', score: score(4, 1, 0) },
      { text: 'Разбираюсь, почему штука устроена именно так, а не иначе', score: score(0, 4, 1) },
      { text: 'Проверяю гипотезу и хочу увидеть результат своими глазами', score: score(0, 1, 4) },
    ],
  },
  {
    q: 'Какой вайб проекта тебе ближе?',
    options: [
      { text: 'Умный, цифровой, с данными, визуализацией и вау-эффектом', score: score(4, 1, 0) },
      { text: 'Точный, инженерный, где всё держится на расчёте и логике', score: score(1, 4, 1) },
      { text: 'Лабораторный, живой, где всё решает эксперимент и состав', score: score(0, 1, 4) },
    ],
  },
  {
    q: 'Мем, который про тебя:',
    options: [
      { text: 'Сейчас ещё один тест, и я точно всё пойму', score: score(4, 0, 0) },
      { text: 'Работает? Не трогай. Не работает? Сейчас разберёмся.', score: score(0, 4, 0) },
      { text: 'Если что-то поменяло цвет, день уже удался', score: score(0, 0, 4) },
    ],
  },
  {
    q: 'На экскурсии тебя зацепит в первую очередь...',
    options: [
      { text: 'Место, где данные превращаются в понятные решения', score: score(4, 1, 0) },
      { text: 'Стенд, где можно рассмотреть механизм, прибор или установку', score: score(0, 4, 1) },
      { text: 'Лаборатория, где прямо сейчас что-то смешивают и проверяют', score: score(0, 1, 4) },
    ],
  },
  {
    q: 'Если делать мини-стартап, то что в основе?',
    options: [
      { text: 'Сервис, который анализирует, подсказывает и экономит время', score: score(4, 0, 1) },
      { text: 'Устройство или технология, которую можно собрать и показать', score: score(1, 4, 0) },
      { text: 'Новый материал, состав или способ получить полезный эффект', score: score(0, 1, 4) },
    ],
  },
  {
    q: 'Что тебе интереснее: найти закономерность, собрать систему или получить новый результат?',
    options: [
      { text: 'Найти закономерность и понять, как это можно использовать', score: score(4, 0, 0) },
      { text: 'Собрать систему, в которой каждая деталь на своём месте', score: score(1, 4, 1) },
      { text: 'Получить новый результат и проверить, повторится ли он снова', score: score(0, 1, 4) },
    ],
  },
  {
    q: 'Какой "рабочий стол" кажется самым уютным?',
    options: [
      { text: 'Ноутбук, заметки с идеями и вкладки с анализом', score: score(4, 1, 0) },
      { text: 'Чертежи, инструменты и техника, которую хочется настроить', score: score(0, 4, 1) },
      { text: 'Чистая лабораторная зона, весы и всё по баночкам', score: score(0, 1, 4) },
    ],
  },
  {
    q: 'Какой результат радует сильнее всего?',
    options: [
      { text: 'Когда из сложного хаоса вдруг складывается точный вывод', score: score(4, 0, 1) },
      { text: 'Когда конструкция или схема работает именно так, как задумано', score: score(0, 4, 1) },
      { text: 'Когда опыт даёт чёткий эффект, который можно подтвердить', score: score(0, 1, 4) },
    ],
  },
  {
    q: 'Куда пойдешь завтра, если выбирать по интересу?',
    options: [
      { text: 'Туда, где можно разобраться в технологиях будущего и данных', score: score(4, 0, 1) },
      { text: 'Туда, где можно увидеть реальные установки, приборы и процессы', score: score(0, 4, 1) },
      { text: 'Туда, где что-то исследуют, смешивают и получают новое', score: score(0, 1, 4) },
    ],
  },
];
